...
def load_data(
    ...
    """
        ...
        :return dict_price_data: Dictionary containing electricity, gas, and EUA market data
                dict_op_data: Dictionary containing time-series data for 
                              dynamic methanation operations
    """
    print("---Load data")
    ...
    # RL training excludes 'min_train_len' days to maintain space for
    # the day-ahead market price forecast.
...
class Preprocessing():
    ...
    def __init__(
        ...
        # Variables for splitting the training set into randomly selected subsets for
        # episodic learning
        ...
        # for different processes (RL_PtG\env\ptg_gym_env.py for reference)
    ...
    def preprocessing_array(self) -> None:
        ...
        # as well as Day-ahead potential rewards and load identifiers for the
        # entire training and test sets, e.g., e_r_b_train[0, 5, 156] represents the future
        # value of the electricity price [0,-,-] in 4 hours [-,5,-] at the
        # 156ths entry of the electricity price data set
        self.e_r_b_train = np.zeros((3, self.env_config.price_ahead,
                                     (self.dict_price_data['el_price_train'].shape[0] -
                                      self.env_config.price_ahead)))
        self.e_r_b_val = np.zeros((3, self.env_config.price_ahead,
                                   (self.dict_price_data['el_price_val'].shape[0] -
                                    self.env_config.price_ahead)))
        self.e_r_b_test = np.zeros((3, self.env_config.price_ahead,
                                    (self.dict_price_data['el_price_test'].shape[0] -
                                     self.env_config.price_ahead)))

        for i in range(self.env_config.price_ahead):
            p_a = self.env_config.price_ahead
            self.e_r_b_train[0, i, :] = self.dict_price_data['el_price_train'][i:(-p_a + i)]
            self.e_r_b_train[1, i, :] = self.dict_pot_r_b['pot_rew_train'][i:(-p_a + i)]
            self.e_r_b_train[2, i, :] = self.dict_pot_r_b['part_full_b_train'][i:(-p_a + i)]
            self.e_r_b_val[0, i, :] = self.dict_price_data['el_price_val'][i:(-p_a + i)]
            self.e_r_b_val[1, i, :] = self.dict_pot_r_b['pot_rew_val'][i:(-p_a + i)]
            self.e_r_b_val[2, i, :] = self.dict_pot_r_b['part_full_b_val'][i:(-p_a + i)]
            self.e_r_b_test[0, i, :] = self.dict_price_data['el_price_test'][i:(-p_a + i)]
            self.e_r_b_test[1, i, :] = self.dict_pot_r_b['pot_rew_test'][i:(-p_a + i)]
            self.e_r_b_test[2, i, :] = self.dict_pot_r_b['part_full_b_test'][i:(-p_a + i)]

        # g_e: Multi-Dimensional Array which stores Day-ahead gas and
        # EUA price data for the entire training and test set
    ...
    def define_episodes(self) -> None:
        ...
        # self.n_eps: No. of training subsets (episodes)
        self.n_eps = int(self.env_config.train_len_d / self.env_config.eps_len_d)
        # self.eps_len: Episode length in [s]
        self.eps_len = 24 * 3600 * self.env_config.eps_len_d
    ...
    def dict_env_kwargs(self, train_val_test_type: str = "train") -> dict[str, Any]:
        ...
        env_kwargs = {
            ...
            **{key: getattr(self.env_config, key) for key in
            ["noise", "eps_len_d", "sim_step", "time_step_op", "price_ahead", "scenario",
                ...
                "T_l_b", "T_u_b", "h2_l_b", "h2_u_b", "ch4_l_b", "ch4_u_b", "h2_res_l_b",
                "h2_res_u_b", "h2o_l_b", "h2o_u_b", "heat_l_b", "heat_u_b", "raw_modified"]},
        ...
        if train_val_test_type == "train":
            ...
            if train_val_test_type == "val":
                ...
                raise ValueError(f'Invalid type: {train_val_test_type}. '
...
def config_print(
    ...
    if train_config.model_conf == "simple_train" or train_config.model_conf == "save_model":
        print(f"---Training case details: RL_PtG{train_config.str_inv} |"
              f" {train_config.model_conf} ")
    else:
        print(f"---Training case details: RL_PtG{train_config.str_inv} |"
              f" {train_config.model_conf} | (Pretrained model: "
              "RL_PtG{train_config.str_inv_load}) ")
    str_id = "RL_PtG_" + train_config.str_inv
    if env_config.scenario == 1:
        print("    > Business case (_BS):\t\t\t 1 -"
              " Trading at the electricity, gas, and emission spot markets")
    elif env_config.scenario == 2:
        print("    > Business case  (_BS):\t\t\t 2 -"
              " Fixed synthetic natural gas (SNG) price and trading at the"
              " electricity and emission spot markets")
    else:
        print("    > Business case (_BS):\t\t\t 3 -"
              " Participating in EEG tenders by using a CHP plant and trading"
              " at the electricity spot markets")
    ...
    if env_config.raw_modified == 'raw':
        print("    > State feature design (_sf) :\t\t "
              "Raw energy market data (electricity, gas, and EUA market signals)")
    else:
        print("    > State feature design (_sf) :\t\t "
              "Modified energy market data (potential reward and load identifier)")
...
def create_vec_envs(
    ...
    # Set processing type
    if train_config.parallel == "Singleprocessing":
        # DummyVecEnv -> computes each workers interaction in serial,
        #                if calculating the env itself is quite fast
        vec_env_cls = DummyVecEnv
    elif train_config.parallel == "Multiprocessing":
        # SubprocVecEnv for multiprocessing -> computes each workers interaction in parallel,
        #                                      if calculating the env itself is quite slow
        vec_env_cls = SubprocVecEnv
    else:
        assert False, ('Choose either "Singleprocessing" or "Multiprocessing"'
                       ' in RL_PTG/config/config_train.yaml -> parallel!')
    ...
    env_train = _make_env(env_id, agent_config.n_envs, train_config.seed_train,
                          dict(dict_input=env_kwargs_data['env_kwargs_train'],
                               train_or_eval=train_config.train_or_eval,
                               render_mode="None"),
                          vec_env_cls)

    # Create callbacks for validation and test
    # pylint: disable=too-many-function-args, redundant-keyword-arg
    _, eval_callback_val = _make_eval_env(env_id, str_id, train_config, agent_config,
                                          env_kwargs_data['env_kwargs_val'], "val")
    _, eval_callback_test = _make_eval_env(env_id, str_id, train_config, agent_config,
                                           env_kwargs_data['env_kwargs_test'], "test")

    # Create second test environment with only one instance of the environment for postprocessing
    env_test_post, _ = _make_eval_env(env_id, str_id, train_config, agent_config,
                                      env_kwargs_data['env_kwargs_test'], "test_post", n_envs=1)
...
class Postprocessing():
    ...
    def test_performance(self) -> None:
        ...
        for m, stats_n in enumerate(self.env_config.stats_names):
            self.stats_dict_test[stats_n] = stats[:(timesteps), m]
    ...
    def plot_results(self) -> None:
        ...
        axs1_1 = axs[1].twinx()
        axs1_1.plot(time_sim, stats_dict['Meth_CH4_flow_stats'][:-6]*1000,
                    'yellowgreen', label='CH$_4$')
        ...
        axs2_1 = axs[2].twinx()
        axs2_1.plot(time_sim, stats_dict['Meth_cum_reward_stats'][:-6]/100,
                    'k', label='Cum. Reward')
        axs2_1.set_ylabel('Cumulative \n reward [€]')
        axs2_1.legend(loc="upper right", fontsize='small')

        fig.suptitle((f"{self.str_id} \n "
                      f"Rew: {np.round(stats_dict['Meth_cum_reward_stats'][-7]/100, 0)} €"),
                     fontsize=9)
