...
def create_vec_envs(env_id, str_id, AgentConfig, TrainConfig):            ######### MAKE SHORTER ########### MAYBE WITH A DECORATOR ################
    """
        ...
        :param TrainConfig: Training configuration in a class object
        :return env_train: Training environment
    ...
    if TrainConfig.parallel == "Singleprocessing":
        ...
        env_train = make_vec_env(env_id=env_id, n_envs=AgentConfig.n_envs, seed=TrainConfig.seed_train, vec_env_cls=DummyVecEnv,
                           env_kwargs=dict(dict_input=env_kwargs_train, train_or_eval=TrainConfig.train_or_eval,
                                           render_mode="None"))
    elif EnvConfig.parallel == "Multiprocessing":
    ...
    env_val = make_vec_env(env_id, n_envs=TrainConfig.eval_trials, seed=TrainConfig.seed_test, vec_env_cls=DummyVecEnv,
                            env_kwargs=dict(dict_input=env_kwargs_test, train_or_eval=TrainConfig.train_or_eval,
                                            render_mode="None"))
...
if __name__ == '__main__':
    ...
    env_kwargs_cv = Preprocess.dict_env_kwargs("val")
    ...
    env_train, eval_callback_val, eval_callback_test = create_vec_envs(env_id, str_id, AgentConfig, TrainConfig)          # Create vectorized environments
    ...
    if TrainConfig.model_conf == "simple_train" or TrainConfig.model_conf == "save_model":          # Train RL model from scratch
        ...
        model = AgentConfig.load_model(env_train, tb_log)
